...
import { FilterQuery } from "mongoose";
 ...
 export interface IContractRepository {
    ...
    getContracts(filter?: FilterQuery<IContract>): Promise<IContract[]>
    ...
    getVendorWrokWithPaginationAndLocation (vendorId:string,serviceType:string,query:{page?:string,limit?:string,search?:string,lat?:number,lng?:number,distance?:number,postedWithin:string}):Promise< {data:ContractDto[], pagination:IPaginationResponseMeta}>
    ...
    getContractStats(vendorId: string, year: number): Promise<MonthlyData[]>
    getAdminContractStats(): Promise<AdminContractStats>
 }