...
class VendorService implements IVendorShopService {
  ...
  getShopData = async (id: string): Promise<VendorDto> => {
    ...
    throw new ErrorResponse(
      MessageEnum.VENDOR__DATA_FETCH_FAILED,
  ...
  getDashboard = async (
    ...
    const bookingStats = await this._bookingRepo.getBookingStats(
      shopId,
      currentYear,
    );
    const contractStats = await this._contractRepo.getContractStats(
      shopId,
      currentYear,
    );
    const weeklyBookingStats =
      await this._bookingRepo.getWeeklyBookingStats(shopId);
    ...
    const totalUnavailableStaff = staffData.reduce(
      (acc: number, data: IStaff) => {
        if (!data.isActive) {
          acc += 1;
        }
        return acc;
    ...
    const totalAvailableService = serviceData.reduce(
      (acc: number, data: IService) => {
        if (data.isActive) {
          acc += 1;
        }
        return acc;
    ...
    const totalUnavailableService = serviceData.reduce(
      (acc: number, data: IService) => {
        if (!data.isActive) {
          acc += 1;
        }
    ...
    const chartData = months.map((month, index) => {
      ...
      const booking = bookingStats.find(
        (b: { _id: number; count: number }) => b._id === monthNum,
      );
      const contract = contractStats.find(
        (c: { _id: number; count: number }) => c._id === monthNum,
      );
      return {
        month: month,
    ...
    const weeklyChartData = days.map((day, index) => {
      const dayNum = index + 1;
        ...
        console.log('weeklyBookingStatus :>> ', weeklyBookingStats);

      const booking = weeklyBookingStats.find((b: {_id:number,count:number , revenue:number}) => b._id === dayNum);
      return {
        day: day,
        bookings: booking ? booking.count : 0,
    ...
    const extraStats =
      await this._bookingRepo.getVendorRevenueAndCustomerCount(shopId);
    const detailedAnalytics =
      await this._bookingRepo.getDetailedVendorAnalytics(shopId);
  ...
  updateVendor = async (
    ...
    const result = await this._vendorRepo.findByIdAndUpdate(
      _id as string,
      updateData,
    );
    if (result) {
      ...
      throw new ErrorResponse(
        MessageEnum.VENDOR_DATA_UPDATION_FAILED,
  ...
  getVendorsData = async (data: {
    ...
    categories?: string[];
    ratings?: string[];
  }): Promise<{ data: VendorDto[]; pagination: IPaginationResponseMeta }> => {
    ...
    const activeShops = (response.data || []).filter(
      (shop) => shop.hasShop == true && shop.isActive == true,
    );
    return {
      data: VendorMapper.toDTOList(activeShops),
      pagination: response.pagination,
    };
  };

  //---------------------------- add shop image
  addShopImages = async (datas: {
    data: IImage[];
    userId: string;
  }): Promise<boolean | void> => {
  ...
  removeImage = async (data: {
    ...
    if (result) {
      ...
      throw new ErrorResponse(
        MessageEnum.VENDOR_SHOP_IMAGE_DELETED_FAILED,
        StatusCodeEnum.INTERNAL_SERVER_ERROR,
      );
    } else {
      logger.error("error to remove shop image");
